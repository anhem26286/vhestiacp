...
upgrade_complete_message() {
	...
	echo "https://github.com/anhem26286/vhestiacp/issues                                  "
	echo
...
upgrade_send_notification_to_email() {
	...
	if [ "$UPGRADE_SEND_EMAIL" = "true" ]; then
		...
		echo "- File an issue report on GitHub: https://github.com/anhem26286/vhestiacp/issues" >> $message_tmp_file
		echo "" >> $message_tmp_file
...
upgrade_init_logging() {
	...
	if [[ "$new_version" =~ "alpha" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Development build for testing purposes only. Report bugs at https://github.com/anhem26286/vhestiacp/issues/."
	fi
	if [[ "$new_version" =~ "beta" ]]; then
		$BIN/v-log-action "system" "Warning" "Updates" "Beta release. Please report bugs at https://github.com/anhem26286/vhestiacp/issues/."
	fi
}
