...
HESTIA="$VHESTIA"  # Backward compatibility alias
export VHESTIA HESTIA
LOG="/root/vhst_install_backups/vhst_install-$(date +%d%m%Y%H%M).log"
memory=$(grep 'MemTotal' /proc/meminfo | tr ' ' '\n' | grep [0-9])
vhst_backups="/root/vhst_install_backups/$(date +%d%m%Y%H%M)"
hst_backups="$vhst_backups"  # Backward compatibility
spinner="/-\|"
os='ubuntu'
...
HESTIA_INSTALL_DIR="$VHESTIA_INSTALL_DIR"  # Backward compatibility
HESTIA_COMMON_DIR="$VHESTIA_COMMON_DIR"    # Backward compatibility
VERBOSE='no'

# VHestiaCP Configuration
# Set your own GitHub repo here to use custom packages
VHESTIACP_REPO="${VHESTIACP_REPO:-}"  # e.g., "yourusername/vhestiacp"
VHESTIACP_BRANCH="${VHESTIACP_BRANCH:-main}"
# VHestiaCP uses clone+build model - no external package repo needed

# Define software versions
VHESTIA_INSTALL_VER='2.0.0'
HESTIA_INSTALL_VER="$VHESTIA_INSTALL_VER"  # Backward compatibility
...
# VHestiaCP: Standalone installation - no hestia, hestia-nginx, hestia-php packages
# Panel runs on PM2 with HTTPS, uses system PHP for CLI scripts
...
help() {
  ...
  --nodejs-versions       Node.js Versions      [18,20,22] default: 20
  -P, --python            Install Python/Gunicorn [yes|no] default: no
  --rabbitmq              Install RabbitMQ      [yes|no]  default: no
  ...
  --github-repo           GitHub repo for packages (e.g., user/vhestiacp)
  --github-release        Use GitHub releases   [yes|no]  default: no
...
# Pre-parse VHestiaCP extended options (they use --option value format)
# These need to be extracted before the main argument translation
all_args=("$@")
remaining_args=()
i=0
...
while [ $i -lt ${#all_args[@]} ]; do
	...
	case "$arg" in
		...
		--mongodb-version)
			...
			;;
		--nodejs-versions)
			nodejs_versions="$next_arg"
			i=$((i+2))
			;;
		--rabbitmq-management)
			rabbitmq_management="$next_arg"
			i=$((i+2))
			;;
		--kafka-ui)
			kafka_ui="$next_arg"
		...
		--debug)
			...
			;;
		--github-repo)
			VHESTIACP_REPO="$next_arg"
			i=$((i+2))
			;;
		--github-release)
			USE_GITHUB_RELEASES="$next_arg"
			i=$((i+2))
			;;
		*)
			remaining_args+=("$arg")
...
for arg; do
	...
	case "$arg" in
		...
		--help) args="${args}-h " ;;
		# VHestiaCP options (simple yes/no flags)
		--haproxy) args="${args}-H " ;;
		--mongodb) args="${args}-O " ;;
		--nodejs) args="${args}-N " ;;
		--python) args="${args}-P " ;;
...
# MongoDB 7.0 doesn't support Ubuntu 24.04 (noble), use 8.0 instead
...
# VHestiaCP: Node.js is required for web_v2 panel, default to yes
nodejs=${nodejs:-yes}
...
while getopts "a:w:v:j:k:m:M:g:d:x:z:Z:c:t:i:b:r:o:q:L:l:y:s:u:e:p:W:D:H:O:N:P:R:K:E:fh" Option; do
	case $Option in
		...
		H) haproxy=$OPTARG ;;       # VHestiaCP: HAProxy
		O) mongodb=$OPTARG ;;       # VHestiaCP: MongoDB
		N) nodejs=$OPTARG ;;        # VHestiaCP: Node.js
...
if [ -n "$haproxy" ] || [ -n "$mongodb" ] || [ -n "$nodejs" ] || [ -n "$python" ] || [ -n "$rabbitmq" ] || [ -n "$kafka" ] || [ -n "$redis" ]; then
	...
	echo "  nodejs=$nodejs"
	echo "  nodejs_versions=$nodejs_versions"
	echo "  python=$python"
...
# VHestiaCP default: HAProxy -> nginx (no Apache)
set_default_value 'nginx' 'yes'
set_default_value 'apache' 'no'
set_default_value 'phpfpm' 'yes'
set_default_value 'haproxy' 'yes'
...
# Welcome message
echo "Welcome to the VHestiaCP Control Panel installer!"
echo
...
# VHestiaCP: No external repo check needed - standalone installation
...
# VHestiaCP: Skip version check against HestiaCP repo
# VHestiaCP is standalone and does not need to match HestiaCP version
echo "[ * ] VHestiaCP v${VHESTIA_INSTALL_VER} - Standalone installation"
...
install_welcome_message() {
	...
	echo '   ╚═══╝  ╚═╝  ╚═╝╚══════╝╚══════╝   ╚═╝   ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝      '
	echo "                                                                        "
	echo "                     VHestia Control Panel v1.0.0                       "
	echo "                      Based on HestiaCP ${DISPLAY_VER}                        "
	...
	echo "                                                                        "
	echo "        Extended: HAProxy | MongoDB | Node.js/PM2 | Python/Gunicorn     "
	echo
	echo "========================================================================"
	echo
	echo "Thank you for choosing VHestia Control Panel! In a few moments,"
...
if [ "$mongodb" = 'yes' ]; then
	echo "   - MongoDB ${mongodb_version} Database Server"
fi

if [ "$nodejs" = 'yes' ]; then
	echo "   - Node.js (v${nodejs_versions}) + PM2 Process Manager"
fi

if [ "$python" = 'yes' ]; then
	echo '   - Python + Pyenv + Gunicorn'
fi

if [ "$rabbitmq" = 'yes' ]; then
	echo -n '   - RabbitMQ Message Broker'
	if [ "$rabbitmq_management" = 'yes' ]; then
		echo -n ' + Management UI'
	fi
	echo
fi

if [ "$kafka" = 'yes' ]; then
	echo -n '   - Apache Kafka Streaming Platform'
...
if [ -z "$username" ]; then
	# If force mode or non-interactive, use default username 'admin'
	if [ "$force" = 'yes' ] || [ "$interactive" = 'no' ]; then
		username="admin"
...
# Ask for password (auto-generate if non-interactive)
if [ -z "$vpass" ]; then
	if [ "$interactive" = 'no' ]; then
		# Auto-generate password for non-interactive install
		vpass=$(gen_pass)
...
# Asking for contact email (auto-generate if non-interactive)
if [ -z "$email" ]; then
	if [ "$interactive" = 'no' ]; then
		# Auto-generate email for non-interactive install
		# Ensure we have a valid domain format
		auto_hostname=$(hostname -f 2>/dev/null || hostname 2>/dev/null || echo 'localhost')
		if [[ ! "$auto_hostname" =~ \. ]]; then
			auto_hostname="${auto_hostname}.local"
		fi
		email="admin@${auto_hostname}"
...
# Asking to set FQDN hostname (auto-detect if non-interactive)
if [ -z "$servername" ]; then
	if [ "$interactive" = 'no' ]; then
		# Auto-detect hostname for non-interactive install
		servername=$(hostname -f 2>/dev/null || hostname 2>/dev/null || echo "server.local")
		# Ensure it's a valid FQDN
		if [[ ! "$servername" =~ \. ]]; then
			servername="${servername}.local"
		fi
		echo "[ * ] Using hostname: $servername"
	else
...
# Validate Hostname in interactive mode
if [ "$interactive" = 'yes' ] && [ -z "$servername" ]; then
	servername=$(hostname -f)
fi

# Validate Hostname, go to loop if the validation fails (only in interactive mode)
if [ "$interactive" = 'yes' ]; then
...
# VHestiaCP: No external repo needed - standalone installation via clone+build
echo "[ * ] VHestiaCP (standalone package)"

# Installing Node.js repo (for web terminal)
if [ "$webterminal" = 'yes' ]; then
	echo "[ * ] Node.js $node_v"
...
# VHestiaCP Extended Components Repositories
if [ "$mongodb" = 'yes' ]; then
	echo "[ * ] MongoDB $mongodb_version (VHestiaCP)"
	curl -fsSL https://www.mongodb.org/static/pgp/server-${mongodb_version}.asc 2>/dev/null | \
		gpg --dearmor -o /usr/share/keyrings/mongodb-server-${mongodb_version}.gpg 2>/dev/null
	echo "deb [ arch=$ARCH signed-by=/usr/share/keyrings/mongodb-server-${mongodb_version}.gpg ] https://repo.mongodb.org/apt/ubuntu $codename/mongodb-org/${mongodb_version} multiverse" > $apt/mongodb-org-${mongodb_version}.list
fi

if [ "$haproxy" = 'yes' ]; then
	echo "[ * ] HAProxy (VHestiaCP)"
	# HAProxy is in Ubuntu repos, no extra repo needed
fi

if [ "$nodejs" = 'yes' ]; then
	echo "[ * ] Node.js/NVM/PM2 (VHestiaCP)"
	# NVM will be installed from GitHub, no apt repo needed
fi

if [ "$python" = 'yes' ]; then
	echo "[ * ] Python/Pyenv/Gunicorn (VHestiaCP)"
	# Pyenv will be installed from GitHub, no apt repo needed
fi
...
# Updating system
if [ "$debug" = 'yes' ]; then
	echo "[ * ] Updating currently installed packages (verbose mode)..."
...
# VHestiaCP: Standalone installation - vhestia package is installed separately after apt packages
# No hestia, hestia-nginx, hestia-php packages from apt
...
# VHestiaCP Extended Packages
if [ "$haproxy" = 'yes' ]; then
	software="$software haproxy"
fi
# Note: MongoDB packages will be installed separately by v-add-sys-mongodb
# to allow for version selection
...
# Show realtime output with tee (both screen and log file)
apt-get -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" install $software 2>&1 | tee -a $LOG

# Check Installation result
if [ ${PIPESTATUS[0]} -ne 0 ]; then
    check_result 1 "apt-get install failed"
fi
...
# Method 3: Install from installer directory (clone+build scenario)
	...
	if ls $DEBS_DIR/vhestia_*.deb 1>/dev/null 2>&1; then
		...
		# Create directories
		mkdir -p $VHESTIA/{bin,func,install,conf,data,log,ssl,web/inc}
		mkdir -p /etc/vhestia

		# Copy files from source
		if [ -d "$SCRIPT_DIR/../bin" ]; then
			cp -rf "$SCRIPT_DIR/../bin"/* $VHESTIA/bin/
			chmod +x $VHESTIA/bin/v-*
		fi
		...
		if [ -f "$SCRIPT_DIR/../web/inc/mail-wrapper.php" ]; then
			cp -f "$SCRIPT_DIR/../web/inc/mail-wrapper.php" $VHESTIA/web/inc/
		fi

		# Create config
		cat > /etc/vhestia/vhestia.conf << 'VHCONF'
# VHestiaCP Configuration
...
VHCONF
		...
		echo "    - VHestiaCP files installed directly"
	fi
fi
...
# Web stack
# When HAProxy is enabled, it takes port 80/443 and backend (nginx or apache) uses 8080/8443
# When HAProxy is disabled, the frontend web server (nginx or apache) uses 80/443

if [ "$haproxy" = 'yes' ]; then
	# HAProxy mode: HAProxy on 80/443, backend on 8080/8443
	if [ "$apache" = 'yes' ]; then
		# HAProxy -> Apache (no nginx proxy layer)
		...
		# No PROXY_SYSTEM - HAProxy replaces nginx as proxy
	...
	write_config_value "HAPROXY_PORT" "80"
	write_config_value "HAPROXY_SSL_PORT" "443"
else
	# No HAProxy: traditional HestiaCP behavior
	if [ "$apache" = 'yes' ]; then
		# nginx (proxy on 80/443) -> apache (backend on 8080/8443)
		write_config_value "WEB_SYSTEM" "apache2"
		write_config_value "WEB_RGROUPS" "www-data"
		write_config_value "WEB_PORT" "8080"
		write_config_value "WEB_SSL_PORT" "8443"
...
if [ "$postgresql" = 'yes' ]; then
	...
	# VHestiaCP: Use local package instead of HestiaCP GitHub
	if [ -f "$HESTIA_INSTALL_DIR/../packages/phppgadmin-v$pga_v.tar.gz" ]; then
		cp $HESTIA_INSTALL_DIR/../packages/phppgadmin-v$pga_v.tar.gz .
	else
		# Fallback to direct phpPgAdmin source
		wget --retry-connrefused --quiet https://github.com/phppgadmin/phppgadmin/releases/download/REL_7-14-6/phpPgAdmin-7.14.6.tar.gz -O phppgadmin-v$pga_v.tar.gz || \
...
# Web terminal
# VHestiaCP: Web terminal is handled by web_v2, not hestia-web-terminal package
if [ "$webterminal" = 'yes' ]; then
...
# because VHestiaCP has modified versions of these scripts
...
# VHestiaCP: Scripts are already installed via vhestia package
# The package postinst creates symlinks and sets up paths correctly
echo "[ * ] VHestiaCP scripts installed via package"
...
# VHestiaCP: Panel runs on PM2, not hestia service
# The web_v2 panel will be set up in the extended features section
echo "[ * ] Setting up VHestiaCP data directories..."
mkdir -p $VHESTIA/data/sessions
chown hestiaweb:hestiaweb $VHESTIA/data/sessions
...
#----------------------------------------------------------#
#              VHestiaCP Extended Features                  #
#----------------------------------------------------------#

echo ""
echo "========================================================================"
echo "[ * ] Configuring VHestiaCP extended features..."
echo "========================================================================"

# VHestiaCP: All bin/func scripts are already installed via vhestia package
# Here we just copy nginx templates and set up extended services

# Determine source directory for templates (where installer is running from)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VHESTIACP_SRC="$(dirname "$SCRIPT_DIR")"

# Copy VHestiaCP nginx templates
if [ -d "$VHESTIACP_SRC/install/deb/templates/web/nginx" ]; then
	echo "[ * ] Installing VHestiaCP nginx templates..."
	mkdir -p $VHESTIA/data/templates/web/nginx/php-fpm
	# Copy root nginx templates
	cp -f $VHESTIACP_SRC/install/deb/templates/web/nginx/*.tpl $VHESTIA/data/templates/web/nginx/ 2>/dev/null
	cp -f $VHESTIACP_SRC/install/deb/templates/web/nginx/*.stpl $VHESTIA/data/templates/web/nginx/ 2>/dev/null
	# Copy php-fpm templates
	if [ -d "$VHESTIACP_SRC/install/deb/templates/web/nginx/php-fpm" ]; then
		cp -f $VHESTIACP_SRC/install/deb/templates/web/nginx/php-fpm/*.tpl $VHESTIA/data/templates/web/nginx/php-fpm/ 2>/dev/null
		cp -f $VHESTIACP_SRC/install/deb/templates/web/nginx/php-fpm/*.stpl $VHESTIA/data/templates/web/nginx/php-fpm/ 2>/dev/null
	fi
	echo "    - Templates installed"
elif [ -d "$VHESTIA/install/deb/templates/web/nginx" ]; then
	# Templates might be in installed location
	echo "[ * ] Using installed nginx templates..."
	mkdir -p $VHESTIA/data/templates/web/nginx/php-fpm
	cp -f $VHESTIA/install/deb/templates/web/nginx/*.tpl $VHESTIA/data/templates/web/nginx/ 2>/dev/null
	cp -f $VHESTIA/install/deb/templates/web/nginx/*.stpl $VHESTIA/data/templates/web/nginx/ 2>/dev/null
	if [ -d "$VHESTIA/install/deb/templates/web/nginx/php-fpm" ]; then
		cp -f $VHESTIA/install/deb/templates/web/nginx/php-fpm/*.tpl $VHESTIA/data/templates/web/nginx/php-fpm/ 2>/dev/null
		cp -f $VHESTIA/install/deb/templates/web/nginx/php-fpm/*.stpl $VHESTIA/data/templates/web/nginx/php-fpm/ 2>/dev/null
	fi
fi

# Show selected options
echo ""
echo "[ * ] VHestiaCP options selected:"
echo "    - HAProxy: ${haproxy:-no}"
echo "    - MongoDB: ${mongodb:-no}"
echo "    - Node.js: ${nodejs:-no}"
echo "    - Python: ${python:-no}"
echo ""
...
# Fix permissions
echo "[ * ] Setting VHestiaCP permissions..."
chmod 755 "$VHESTIA"
chmod 755 "$VHESTIA/conf" 2>/dev/null || true
chmod 755 "$VHESTIA/data" 2>/dev/null || true
chmod 755 "$VHESTIA/func" 2>/dev/null || true
chmod 755 "$VHESTIA/bin" 2>/dev/null || true
chmod +x "$VHESTIA/bin/"v-* 2>/dev/null || true
chmod 644 "$VHESTIA/conf/"*.conf 2>/dev/null || true

# VHestiaCP: All scripts are already installed via vhestia package
# No need to create scripts inline - they're in bin/ directory

#----------------------------------------------------------#
#              Install VHestiaCP Extended Components       #
#----------------------------------------------------------#

# Install HAProxy if requested
if [ "$haproxy" = 'yes' ]; then
	echo "[ * ] Installing HAProxy load balancer..."
	...
	# ============================================
	# Step 2: Change web server ports from 80/443 to 8080/8443
	# HAProxy will take over port 80/443
	# ============================================
	
	# Update hestia.conf - already set in web stack config section above
	# Just ensure values are correct
	sed -i "s/^WEB_PORT=.*/WEB_PORT='8080'/" "$HESTIA/conf/hestia.conf"
	sed -i "s/^WEB_SSL_PORT=.*/WEB_SSL_PORT='8443'/" "$HESTIA/conf/hestia.conf"
	
	if [ "$apache" = 'yes' ]; then
		# ============================================
		# HAProxy -> Apache: Update Apache ports
		# ============================================
		echo "    - Changing Apache ports to 8080/8443..."
		...
		for conf in /etc/apache2/sites-available/*.conf /etc/apache2/sites-enabled/*.conf; do
			if [ -f "$conf" ]; then
				...
				sed -i 's/:443>/:8443>/g' "$conf"
				perl -pi -e 's/<VirtualHost\s+\*:80>/<VirtualHost *:8080>/g' "$conf"
				perl -pi -e 's/<VirtualHost\s+\*:443>/<VirtualHost *:8443>/g' "$conf"
				perl -pi -e 's/<VirtualHost\s+\d+\.\d+\.\d+\.\d+:80>/<VirtualHost *:8080>/g' "$conf"
				perl -pi -e 's/<VirtualHost\s+\d+\.\d+\.\d+\.\d+:443>/<VirtualHost *:8443>/g' "$conf"
			fi
		done
		...
		# Verify
		if ss -tlnp | grep -q ":8080.*apache"; then
			echo "    - ✓ Apache now on port 8080"
		else
			echo "    - ⚠ Apache port change may have failed"
		fi
		
		# Disable nginx if it was installed as proxy (we don't need it with HAProxy)
		if systemctl is-active --quiet nginx; then
			echo "    - Stopping nginx (HAProxy replaces it as frontend)..."
		...
		# ============================================
		# HAProxy -> nginx: Update nginx ports
		# ============================================
		echo "    - Changing nginx ports to 8080/8443..."
		
		# Update ALL nginx configs - use perl for reliable regex
		# Change IP:80 -> 0.0.0.0:8080 and IP:443 -> 0.0.0.0:8443
		for conf in /etc/nginx/conf.d/*.conf; do
			if [ -f "$conf" ]; then
				perl -pi -e 's/listen\s+\d+\.\d+\.\d+\.\d+:80\b/listen 0.0.0.0:8080/g' "$conf"
				perl -pi -e 's/listen\s+80\b/listen 8080/g' "$conf"
				perl -pi -e 's/listen\s+\[::\]:80\b/listen [::]:8080/g' "$conf"
				perl -pi -e 's/listen\s+\d+\.\d+\.\d+\.\d+:443\b/listen 0.0.0.0:8443/g' "$conf"
				perl -pi -e 's/listen\s+443\b/listen 8443/g' "$conf"
				perl -pi -e 's/listen\s+\[::\]:443\b/listen [::]:8443/g' "$conf"
			fi
		done
		...
		# Force fix any remaining with sed
		...
		# Restart nginx
		echo "    - Restarting nginx on port 8080..."
		...
		# Verify nginx is on 8080
		if ss -tlnp | grep -q ":8080.*nginx"; then
			echo "    - ✓ nginx now on port 8080"
		else
			echo "    - ⚠ nginx port change may have failed"
		fi
	fi
	
	# ============================================
	# Step 3: Kill any remaining processes on port 80/443
	# ============================================
	fuser -k 80/tcp 2>/dev/null || true
	fuser -k 443/tcp 2>/dev/null || true
	sleep 1
	...
	# ============================================
	# Step 4: Create HAProxy config with frontend/backend
	# ============================================
...
#---------------------------------------------------------------------
# Web SSL Backend (port 8443) - TCP passthrough
#---------------------------------------------------------------------
...
ERROR_EOF
	...
	if ss -tlnp | grep -q ":80.*haproxy"; then
		echo "    - ✓ HAProxy now on port 80"
	else
		echo "    - ⚠ HAProxy may not be on port 80"
		ss -tlnp | grep ":80"
	fi
	...
	iptables -I INPUT -p tcp --dport 8404 -j ACCEPT 2>/dev/null || true
	
	echo "    - HAProxy installed successfully"
	echo "    - Stats: http://$(hostname -I | awk '{print $1}'):8404/stats"
	echo "    - Stats user: admin"
	echo "    - Stats pass: $HAPROXY_STATS_PASS"
fi

# Install MongoDB if requested
if [ "$mongodb" = 'yes' ]; then
	echo "[ * ] Installing MongoDB ${mongodb_version}..."
	if [ -f "$HESTIA/bin/v-add-sys-mongodb" ]; then
		$HESTIA/bin/v-add-sys-mongodb "$mongodb_version" 2>&1 | tee -a $LOG
		if [ $? -eq 0 ]; then
			write_config_value "MONGODB_SYSTEM" "yes"
			write_config_value "MONGODB_VERSION" "$mongodb_version"
			echo "    - MongoDB installed successfully"
			
			# CRITICAL: Fix mongodb.conf permission so web panel can read it
			# hestiaweb user runs PHP and needs to read this file
			if [ -f "$HESTIA/conf/mongodb.conf" ]; then
				chmod 644 "$HESTIA/conf/mongodb.conf"
				echo "    - Fixed mongodb.conf permissions (644)"
			fi
			
			# Add hestiaweb to admin group so it can read mongodb.conf
			if id "hestiaweb" &>/dev/null; then
				usermod -aG admin hestiaweb 2>/dev/null || true
				echo "    - Added hestiaweb to admin group"
			fi
		else
			echo "    - MongoDB installation failed"
		fi
	else
		echo "    - ERROR: v-add-sys-mongodb script not found!"
	fi
fi

# Install Node.js if requested
if [ "$nodejs" = 'yes' ]; then
	echo "[ * ] Installing Node.js with NVM and PM2..."
	if [ -f "$HESTIA/bin/v-add-sys-nodejs" ]; then
		# Parse versions
		IFS=',' read -ra NODE_VERSIONS <<< "$nodejs_versions"
		first="yes"
		for version in "${NODE_VERSIONS[@]}"; do
			version=$(echo $version | tr -d ' ')
			echo "    - Installing Node.js v$version..."
			if [ "$first" = "yes" ]; then
				$HESTIA/bin/v-add-sys-nodejs "$version" yes 2>&1 | tee -a $LOG
				first="no"
			else
				$HESTIA/bin/v-add-sys-nodejs "$version" no 2>&1 | tee -a $LOG
			fi
		done
		write_config_value "NODEJS_SYSTEM" "yes"
		echo "    - Node.js installed successfully"
		
		# Verify symlinks are working for all users
		echo "    - Verifying system-wide access..."
		if [ -x "/usr/local/bin/node" ] && [ -x "/usr/local/bin/npm" ] && [ -x "/usr/local/bin/pm2" ]; then
			echo "      ✓ node: $(/usr/local/bin/node -v)"
			echo "      ✓ npm: $(/usr/local/bin/npm -v)"
			echo "      ✓ pm2: $(/usr/local/bin/pm2 -v 2>/dev/null | head -1)"
			echo "    - All users can now use: node, npm, pm2"
		else
...
if [ "$python" = 'yes' ]; then
	echo "[ * ] Installing Python with Pyenv and Gunicorn..."
	...
	if [ ! -d "/opt/pyenv" ]; then
		...
		cat > /etc/profile.d/pyenv.sh << 'PYENV_EOF'
export PYENV_ROOT="/opt/pyenv"
export PATH="$PYENV_ROOT/bin:$PATH"
eval "$(pyenv init -)"
PYENV_EOF
	...
	pyenv install 3.11 -s 2>&1 | tee -a $LOG
	pyenv global 3.11
	
	# Install gunicorn
	pip install gunicorn 2>&1 | tee -a $LOG
	
	write_config_value "PYTHON_SYSTEM" "yes"
	write_config_value "PYTHON_VERSIONS" "3.11"
fi

# Install RabbitMQ if requested
if [ "$rabbitmq" = 'yes' ]; then
	echo "[ * ] Installing RabbitMQ Message Broker..."
	if [ -f "$HESTIA/bin/v-add-sys-rabbitmq" ]; then
		$HESTIA/bin/v-add-sys-rabbitmq "$rabbitmq_management" 2>&1 | tee -a $LOG
		if [ $? -eq 0 ]; then
			write_config_value "RABBITMQ_SYSTEM" "yes"
			write_config_value "RABBITMQ_MANAGEMENT" "$rabbitmq_management"
			echo "    - RabbitMQ installed successfully"
			
			# Fix permissions for web panel access
			if [ -f "$HESTIA/conf/rabbitmq.conf" ]; then
				chmod 644 "$HESTIA/conf/rabbitmq.conf"
				echo "    - Fixed rabbitmq.conf permissions (644)"
			fi
		else
			echo "    - RabbitMQ installation failed"
		fi
	else
		echo "    - ERROR: v-add-sys-rabbitmq script not found!"
	fi
fi

# Install Kafka if requested
if [ "$kafka" = 'yes' ]; then
	echo "[ * ] Installing Apache Kafka..."
	if [ -f "$HESTIA/bin/v-add-sys-kafka" ]; then
		$HESTIA/bin/v-add-sys-kafka "$kafka_ui" 2>&1 | tee -a $LOG
		if [ $? -eq 0 ]; then
			write_config_value "KAFKA_SYSTEM" "yes"
			write_config_value "KAFKA_UI" "$kafka_ui"
			echo "    - Apache Kafka installed successfully"
			
			# Fix permissions for web panel access
			if [ -f "$HESTIA/conf/kafka.conf" ]; then
				chmod 644 "$HESTIA/conf/kafka.conf"
				echo "    - Fixed kafka.conf permissions (644)"
			fi
		else
			echo "    - Apache Kafka installation failed"
		fi
	else
...
# Add VHestiaCP version marker
write_config_value "VHESTIACP_VERSION" "$VHESTIA_INSTALL_VER"
...
srcdir=""
for try_path in "$VHESTIACP_SRC" "/tmp/vhestiacp" "$HOME/vhestiacp" "/root/vhestiacp"; do
...
if [ -n "$srcdir" ] && [ -d "$srcdir/web_v2" ]; then
	...
	mkdir -p "$VHESTIA/web_v2"

	# Copy server files
	cp -r "$srcdir/web_v2/server" "$VHESTIA/web_v2/"
	cp "$srcdir/web_v2/ecosystem.config.cjs" "$VHESTIA/web_v2/"
	...
	cp -r "$srcdir/web_v2/client" "$VHESTIA/web_v2/"

	echo "    - Installing client dependencies..."
	cd "$VHESTIA/web_v2/client"
	npm install > /dev/null 2>&1

	echo "    - Building client (this may take a few minutes)..."
	npm run build > /dev/null 2>&1

	if [ -d "$VHESTIA/web_v2/client/dist" ]; then
...
#----------------------------------------------------------#
#              Set vhestia.conf default values             #
#----------------------------------------------------------#

echo "[ * ] Updating configuration files..."
BIN="$VHESTIA/bin"
source $VHESTIA/func/syshealth.sh
syshealth_repair_system_config

# Add /usr/local/vhestia/bin/ to path variable
echo 'if [ "${PATH#*/usr/local/vhestia/bin*}" = "$PATH" ]; then
    . /etc/profile.d/vhestia.sh
fi' >> /root/.bashrc

# Create vhestia.sh profile
cat > /etc/profile.d/vhestia.sh << 'EOF'
export VHESTIA='/usr/local/vhestia'
export HESTIA="$VHESTIA"
export PATH="$PATH:$VHESTIA/bin"
EOF
chmod 644 /etc/profile.d/vhestia.sh

#----------------------------------------------------------#
#                   VHestiaCP Access Info                  #
#----------------------------------------------------------#
...
You have successfully installed VHestiaCP (Hestia Control Panel Extended) on your server.
...
# Add VHestiaCP info
if [ "$haproxy" = 'yes' ]; then
	echo "	HAProxy Stats: http://$ip:8404/stats" >> $tmpfile
fi
if [ "$mongodb" = 'yes' ]; then
	echo "	MongoDB: Installed (v$mongodb_version)" >> $tmpfile
fi
if [ "$nodejs" = 'yes' ]; then
	echo "	Node.js: Installed (v$nodejs_versions)" >> $tmpfile
fi
if [ "$python" = 'yes' ]; then
	echo "	Python: Installed (v3.11)" >> $tmpfile
fi

echo -e "
Thank you for choosing VHestiaCP to power your full stack web server!
